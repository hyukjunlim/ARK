"""
Complete O Adsorption Analysis Pipeline with OC20 Energies (Phases 1-3)

O adsorption energy calculation using OC20 predefined atomic energies:
 ΔE_O = E_adslab - E_slab - E_O

Where:
 - E_O = -7.204 eV (OC20 atomic energy)
 - Reference = E_O = -7.204 eV

Optimal value: ΔE_O = 1.5 eV (±0.5 eV discovery range)

This script:
- Phase 1: Load O data and calculate adsorption energies using OC20
...
class OAnalysisOC20:
    """Complete O adsorption analysis pipeline using OC20 atomic energies."""
    ...
    def __init__(self, base_sim_path: str, output_dir: str, arial_font_path: str = None):
        """Initialize O analysis pipeline."""
        self.base_sim_path = Path(base_sim_path)
        self.output_dir = Path(output_dir)
        self.arial_font_path = arial_font_path

        # Create output directories
        self.results_dir = self.output_dir / "results_o"
        self.svg_dir = self.output_dir / "plot_svg_o"
        self.png_dir = self.output_dir / "plot_png_o"
        self.metrics_dir = self.output_dir / "metrics_o"
        self.dft_dir = self.output_dir / "materials_for_dft_o"

        for d in [self.results_dir, self.svg_dir, self.png_dir, self.metrics_dir, self.dft_dir]:
            d.mkdir(parents=True, exist_ok=True)

        # Load font (Arial if available, else DejaVu Sans)
        ...
        self.O_OPT = 1.5
        self.RANGE = 0.5
        self.O_XLIM = (self.O_OPT - 2, self.O_OPT + 2)
        self.O_YLIM = (self.O_OPT - 2, self.O_OPT + 2)

        # OC20 reference energy for O: E_O
        self.ref_energy_oc20 = self.ADSORBATE_ENERGIES['O']

    # ==================== PHASE 1: DATA LOADING ====================

    def phase1_load_data(self):
        """Phase 1: Load and process O data."""
        print("\n" + "=" * 80)
        print("PHASE 1: O DATA LOADING AND PREPROCESSING (OC20 METHOD)")
        print("=" * 80)
        print(f"\nUsing OC20 predefined atomic energies:")
        print(f"  O: {self.ADSORBATE_ENERGIES['O']} eV")
        print(f"  O reference: {self.ref_energy_oc20:.3f} eV")
        print("=" * 80)
        ...
        # Load O adslab data
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Loading O adslab data...")
        o_dir = self.base_sim_path / "O"
        chunk_files = sorted(glob.glob(str(o_dir / "chunk_*" / "O_adslab_results_chunk*.json")))
        ...
        for chunk_file in chunk_files:
            with open(chunk_file) as f:
                ...
                self.o_adslab_data.update(chunk_data)

        print(f"  Loaded {len(self.o_adslab_data)} surfaces")

        # Calculate adsorption energies
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Calculating O adsorption energies (OC20 method)...")
        self.o_results = self.calculate_o_adsorption_energies()
        print(f"  Processed {len(self.o_results)} configurations")

        # Save to CSV
        self.o_df = pd.DataFrame(self.o_results)
        self.o_df.to_csv(self.results_dir / "o_adsorption_data_oc20.csv", index=False)
        print(f"  Saved: o_adsorption_data_oc20.csv")

        # Process min and mean
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Processing min and mean energies...")
        self.o_min = self.get_min_energy_data()
        self.o_mean = self.get_mean_energy_data()

        self.o_min.to_csv(self.results_dir / "o_min_energy_oc20.csv", index=False)
        self.o_mean.to_csv(self.results_dir / "o_mean_energy_oc20.csv", index=False)
        print(f"  Saved: o_min_energy_oc20.csv ({len(self.o_min)} surfaces)")
        print(f"  Saved: o_mean_energy_oc20.csv ({len(self.o_mean)} surfaces)")
    ...
    def calculate_o_adsorption_energies(self):
        """Calculate O adsorption energies using OC20: ΔE = E_adslab - E_slab - E_O."""
        results = []

        for surface_id, surface_data in self.o_adslab_data.items():
    ...
    def get_min_energy_data(self):
        ...
        for surface_id in self.o_df['surface_id'].unique():
            surf_data = self.o_df[self.o_df['surface_id'] == surface_id]
    ...
    def calculate_statistics(self):
        ...
        print("\n" + "=" * 60)
        print("O ADSORPTION STATISTICS (OC20 METHOD)")
        print("=" * 60)

        total = len(self.o_df)
        crack_conv = self.o_df['crack_converged'].sum()
        gemnet_conv = self.o_df['gemnet_converged'].sum()
        both_conv = (self.o_df['crack_converged'] & self.o_df['gemnet_converged']).sum()
        ...
        # Error statistics
        both = self.o_df[self.o_df['crack_converged'] & self.o_df['gemnet_converged']].copy()
        ...
        for calc in ['crack', 'gemnet']:
            conv_df = self.o_df[self.o_df[f'{calc}_converged']]
            active = conv_df[
                (conv_df[f'{calc}_e_ads'] >= self.O_OPT - self.RANGE) &
                (conv_df[f'{calc}_e_ads'] <= self.O_OPT + self.RANGE)
            ]
    ...
    def plot_01_histogram_min(self):
        """O histogram with min energy."""
        fig, ax = plt.subplots(figsize=(10, 7))

        crack_data = self.o_min[self.o_min['crack_converged']]['crack_e_ads']
        gemnet_data = self.o_min[self.o_min['gemnet_converged']]['gemnet_e_ads']

        ax.hist(crack_data, bins=80, range=self.O_XLIM, color=self.color_crack, alpha=0.5,
                edgecolor=edge_color, linewidth=0.5, label='Crack')
        ax.hist(gemnet_data, bins=80, range=self.O_XLIM, color=self.color_gemnet, alpha=0.5,
                edgecolor=edge_color, linewidth=0.5, label='GemNet')
        ax.axvline(self.O_OPT, color='red', linestyle='--', linewidth=2, label='Optimal')
        ax.axvspan(self.O_OPT - self.RANGE, self.O_OPT + self.RANGE, alpha=0.2, color='red')

        ax.set_xlim(self.O_XLIM)
        ax.set_xlabel(f'ΔE{subscript("O")} (eV)')
        ax.set_ylabel('Count')
        ax.set_title('O Adsorption Energy Distribution (Min per Slab, OC20)')
        ax.legend(prop=self.arial_font, fontsize=fss, frameon=True, edgecolor=edge_color)
        ax.grid(True, alpha=0.3, linestyle='--', axis='y')

        self.style_ax(ax)
        plt.tight_layout()
        self.save_plot(fig, "o_01_histogram_min")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_01_histogram_min")

    def plot_02_histogram_mean(self):
        """O histogram with mean energy."""
        fig, ax = plt.subplots(figsize=(10, 7))

        crack_data = self.o_mean[self.o_mean['crack_converged']]['crack_e_ads']
        gemnet_data = self.o_mean[self.o_mean['gemnet_converged']]['gemnet_e_ads']
        ...
        ax.set_xlabel(f'ΔE{subscript("O")} (eV)')
        ax.set_ylabel('Count')
        ax.set_title('O Adsorption Energy Distribution (Mean per Slab, OC20)')
        ax.legend(prop=self.arial_font, fontsize=fss, frameon=True, edgecolor=edge_color)
        ax.grid(True, alpha=0.3, linestyle='--', axis='y')

        self.style_ax(ax)
        plt.tight_layout()
        self.save_plot(fig, "o_02_histogram_mean")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_02_histogram_mean")

    def plot_03_parity_min(self):
        """Parity plot with min energy."""
        fig, ax = plt.subplots(figsize=(9, 9))

        both = self.o_min[self.o_min['crack_converged'] & self.o_min['gemnet_converged']]

        ax.scatter(both['crack_e_ads'], both['gemnet_e_ads'], c=self.color_crack,
                   s=20, alpha=0.5, edgecolors='none')
        ax.plot(self.O_XLIM, self.O_YLIM, 'k--', linewidth=2, label='Perfect agreement')

        ax.set_xlim(self.O_XLIM)
        ax.set_ylim(self.O_YLIM)
        ax.set_xlabel(f'Crack ΔE{subscript("O")} (eV)')
        ax.set_ylabel(f'GemNet ΔE{subscript("O")} (eV)')
        ax.set_title('O: Crack vs GemNet (Min per Slab, OC20)')
        ax.legend(prop=self.arial_font, fontsize=fss, frameon=True, edgecolor=edge_color)
        ax.grid(True, alpha=0.3, linestyle='--')

        self.style_ax(ax)
        plt.tight_layout()
        self.save_plot(fig, "o_03_parity_min")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_03_parity_min")

    def plot_04_parity_mean(self):
        """Parity plot with mean energy."""
        fig, ax = plt.subplots(figsize=(9, 9))

        both = self.o_mean[self.o_mean['crack_converged'] & self.o_mean['gemnet_converged']]
        ...
        ax.set_xlabel(f'Crack ΔE{subscript("O")} (eV)')
        ax.set_ylabel(f'GemNet ΔE{subscript("O")} (eV)')
        ax.set_title('O: Crack vs GemNet (Mean per Slab, OC20)')
        ax.legend(prop=self.arial_font, fontsize=fss, frameon=True, edgecolor=edge_color)
        ax.grid(True, alpha=0.3, linestyle='--')

        self.style_ax(ax)
        plt.tight_layout()
        self.save_plot(fig, "o_04_parity_mean")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_04_parity_mean")

    def plot_05_error_analysis_min(self):
        """Error analysis with min energy."""
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 7))

        both = self.o_min[self.o_min['crack_converged'] & self.o_min['gemnet_converged']].copy()
        ...
        ax1.axvline(self.O_OPT, color='red', linestyle='--', linewidth=2, label='Optimal')
        ax1.set_xlim(self.O_XLIM)
        ax1.set_xlabel(f'Crack ΔE{subscript("O")} (eV)')
        ...
        plt.tight_layout()
        self.save_plot(fig, "o_05_error_analysis_min")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_05_error_analysis_min")

    def plot_06_error_analysis_mean(self):
        """Error analysis with mean energy."""
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 7))

        both = self.o_mean[self.o_mean['crack_converged'] & self.o_mean['gemnet_converged']].copy()
        both['error'] = np.abs(both['crack_e_ads'] - both['gemnet_e_ads'])
        ...
        plt.tight_layout()
        self.save_plot(fig, "o_06_error_analysis_mean")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_06_error_analysis_mean")
    ...
    def plot_07_discovery_timeline(self):
        ...
        for calc, color in [('crack', self.color_crack), ('gemnet', self.color_gemnet)]:
            conv_df = self.o_min[self.o_min[f'{calc}_converged']].copy()
            active = conv_df[
                (conv_df[f'{calc}_e_ads'] >= self.O_OPT - self.RANGE) &
                (conv_df[f'{calc}_e_ads'] <= self.O_OPT + self.RANGE)
            ]
        ...
        ax.set_ylabel('Active O Catalysts Discovered')
        ax.set_title('O Discovery Timeline (OC20)')
        ax.legend(prop=self.arial_font, fontsize=fss, frameon=True, edgecolor=edge_color)
        ax.grid(True, alpha=0.3, linestyle='--')

        self.style_ax(ax)
        plt.tight_layout()
        self.save_plot(fig, "o_07_discovery_timeline")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_07_discovery_timeline")
    ...
    def plot_08_active_catalysts(self):
        ...
        for calc in ['crack', 'gemnet']:
            conv_df = self.o_min[self.o_min[f'{calc}_converged']]
            active = conv_df[
                (conv_df[f'{calc}_e_ads'] >= self.O_OPT - self.RANGE) &
                (conv_df[f'{calc}_e_ads'] <= self.O_OPT + self.RANGE)
            ]
        ...
        ax.set_title(f'O Active Catalysts (ΔE = {self.O_OPT} ± {self.RANGE} eV, OC20)')
        ax.grid(True, alpha=0.3, linestyle='--', axis='y')
        ...
        self.style_ax(ax)
        plt.tight_layout()
        self.save_plot(fig, "o_08_active_catalysts")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_08_active_catalysts")
    ...
    def plot_09_classification_performance(self):
        ...
        for calc, color in [('crack', self.color_crack), ('gemnet', self.color_gemnet)]:
            ...
            ax1.hist(conv_df[f'{calc}_e_ads'], bins=100, range=self.O_XLIM,
                    color=color, alpha=0.3, edgecolor=edge_color, linewidth=0.5,
                    label=f'{calc.capitalize()} (all)')
            ax1.hist(active[f'{calc}_e_ads'], bins=100, range=self.O_XLIM,
                    color=color, alpha=0.7, edgecolor=edge_color, linewidth=0.5,
                    label=f'{calc.capitalize()} (active)')

        ax1.axvline(self.O_OPT, color='red', linestyle='--', linewidth=2, label='Optimal')
        ax1.axvspan(self.O_OPT - self.RANGE, self.O_OPT + self.RANGE, alpha=0.2, color='red')
        ax1.set_xlim(self.O_XLIM)
        ax1.set_xlabel(f'ΔE{subscript("O")} (eV)')
        ax1.set_ylabel('Count')
        ax1.set_title('O: Active vs All Catalysts')
        ax1.legend(prop=self.arial_font, fontsize=fss, frameon=True, edgecolor=edge_color)
        ...
        ax2.set_ylabel('Count')
        ax2.set_title('O: Catalyst Classification')
        ...
        plt.tight_layout()
        self.save_plot(fig, "o_09_classification_performance")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_09_classification_performance")
    ...
    def plot_10_energy_landscape(self):
        ...
        plot_data = both[
            (both['crack_e_ads'] >= self.O_XLIM[0]) &
            (both['crack_e_ads'] <= self.O_XLIM[1]) &
            (both['gemnet_e_ads'] >= self.O_YLIM[0]) &
            (both['gemnet_e_ads'] <= self.O_YLIM[1])
        ]
        ...
        # Optimal region box
        from matplotlib.patches import Rectangle
        rect = Rectangle((self.O_OPT - self.RANGE, self.O_OPT - self.RANGE),
        ...
        ax.set_xlabel(f'Crack ΔE{subscript("O")} (eV)')
        ax.set_ylabel(f'GemNet ΔE{subscript("O")} (eV)')
        ax.set_title('O Energy Landscape Topology (OC20)')
        ax.legend(prop=self.arial_font, fontsize=fss, frameon=True, edgecolor=edge_color)
        ...
        self.style_ax(ax)
        plt.tight_layout()
        self.save_plot(fig, "o_10_energy_landscape")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_10_energy_landscape")
    ...
    def plot_11_cost_benefit(self):
        ...
        ax.set_title('O: Discovery Yield vs Computational Cost (OC20)')
        ax.legend(prop=self.arial_font, fontsize=fss, frameon=True, edgecolor=edge_color)
        ax.grid(True, alpha=0.3, linestyle='--')

        self.style_ax(ax)
        plt.tight_layout()
        self.save_plot(fig, "o_11_cost_benefit")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_11_cost_benefit")
    ...
    def plot_12_material_stratification(self):
        ...
        ax1.set_title('O: Error vs Sample Size by Material')
        ax1.set_xscale('log')
        ax1.grid(True, alpha=0.3, linestyle='--')
        ...
        ax2.set_title('O: Top 20 Worst Materials')
        ...
        plt.tight_layout()
        self.save_plot(fig, "o_12_material_stratification")
        print(f"  [{datetime.now().strftime('%H:%M:%S')}] Saved: o_12_material_stratification")

    # ==================== MAIN EXECUTION ====================

    def run_complete_analysis(self):
        """Run complete O analysis pipeline."""
        print("\n" + "=" * 80)
        print("O ADSORPTION ANALYSIS WITH OC20 ENERGIES")
        print("=" * 80)
        ...
        print("\n" + "=" * 80)
        print("COMPLETE O ANALYSIS FINISHED!")
        print("=" * 80)
        print(f"\nResults saved to:")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='O adsorption energy analysis with OC20 reference energies')
    ...
    pipeline = OAnalysisOC20(args.sim_path, args.output_dir)
