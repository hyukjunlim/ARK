...
class BaseTrainer(ABC):
    ...
    def load_model(self) -> None:
        ...
        if hasattr(self.model, "teacher_model"):
            ...
            if self.model.teacher_model is not None:
                teacher_params = self.model.teacher_model.num_params + (total_params - model_params)
            else:
                teacher_params = 0
            student_params = total_params - teacher_params
            if distutils.is_master():
                logging.info(
                    f"Loaded {self.model.__class__.__name__} with "
                    f"Teacher: {teacher_params} parameters and "
                    f"Student: {student_params} parameters."
                )
        elif hasattr(self.model, "backbone") and self.model.backbone is not None:
            if hasattr(self.model.backbone, "teacher_model"):
                p_student_backbone = self.model.backbone.student_params
                p_teacher_backbone = self.model.backbone.teacher_params
                p_out_mlp = self.model.backbone.out_mlp_embs_params

                p_backbone_total = sum(
                    p.numel() for p in self.model.backbone.parameters()
                )
                p_heads = total_params - p_backbone_total

                if self.model.backbone.teacher_model is not None:
                    teacher_params = p_teacher_backbone + p_heads
                else:
                    teacher_params = 0
                student_params = p_student_backbone + p_heads
                ...
                if distutils.is_master():
                    ...
                    if p_out_mlp > 0:
                        logging.info(
                            f"(Ignored {p_out_mlp:,} parameters in 'out_mlp_embs')"
                        )
        elif hasattr(self.model, "module") and self.model.module is not None:
            if hasattr(self.model.module, "teacher_model"):
                p_student_backbone = self.model.module.backbone.student_params
                p_teacher_backbone = self.model.module.backbone.teacher_params
                p_out_mlp = self.model.module.backbone.out_mlp_embs_params

                p_backbone_total = sum(
                    p.numel() for p in self.model.module.backbone.parameters()
                )
                p_heads = total_params - p_backbone_total

                if self.model.module.backbone.teacher_model is not None:
                    teacher_params = p_teacher_backbone + p_heads
                else:
                    teacher_params = 0
                student_params = p_student_backbone + p_heads
                ...
                if distutils.is_master():
                    logging.info(
                        f"Loaded {self.model.module.__class__.__name__} with "
                        f"Teacher: {teacher_params:,} parameters and "
                        f"Student: {student_params:,} parameters."
                    )
                    if p_out_mlp > 0:
        ...
        if self.logger is not None:
            ...
            self.logger.log_summary({"num_params": total_params})
    ...
    def load_checkpoint(
        ...
        if hasattr(self.model, "teacher_model") and self.model.teacher_model is not None:
            logging.info("Loading pretrained weights into teacher model.")
            teacher_state_dict = match_state_dict(
                self.model.teacher_model.state_dict(), checkpoint["state_dict"]
            )
            load_state_dict(self.model.teacher_model, teacher_state_dict, strict=strict)
        elif hasattr(self.model, "backbone") and self.model.backbone is not None:
            clean_state_dict = {
            ...
            if self.config["optim"].get("pretrained_weights", True):
                # Pretrained Baseline
                load_state_dict(self.model, clean_state_dict, strict=False)
                logging.info("Successfully loaded weights for student backbone and output heads.")
            
            if hasattr(self.model.backbone, "teacher_model") and self.model.backbone.teacher_model is not None:
                # Load teacher model weights for distillation
                teacher_state_dict = {
                    k.replace("backbone.", "", 1): v
                    for k, v in clean_state_dict.items()
                    if k.startswith("backbone.")
                }
                load_state_dict(self.model.backbone.teacher_model, teacher_state_dict, strict=strict)
                logging.info("Successfully loaded pretrained teacher weights.")
        elif hasattr(self.model, "module") and self.model.module is not None:
            clean_state_dict = {
                k[7:] if k.startswith("module.") else k: v
                for k, v in checkpoint["state_dict"].items()
            ...
            if self.config["optim"].get("pretrained_weights", True):
                # Pretrained Baseline
                load_state_dict(self.model.module, clean_state_dict, strict=False)
                logging.info("Successfully loaded weights for student backbone and output heads.")
            
            if hasattr(self.model.module.backbone, "teacher_model") and self.model.module.backbone.teacher_model is not None:
                # Load teacher model weights for distillation
                teacher_state_dict = {
                    k.replace("backbone.", "", 1): v
                    for k, v in clean_state_dict.items()
                ...
                load_state_dict(self.model.module.backbone.teacher_model, teacher_state_dict, strict=strict)
                logging.info("Successfully loaded pretrained teacher weights.")
                
            # if hasattr(self.model.module, "teacher_model") and self.model.module.teacher_model is not None:
            #     logging.info("Loading pretrained weights into teacher model.")
            #     teacher_state_dict = match_state_dict(
            #         self.model.module.teacher_model.state_dict(), checkpoint["state_dict"]
            #     )
            #     load_state_dict(self.model.module.teacher_model, teacher_state_dict, strict=strict)
        else:
            logging.info("Teacher model not found. Loading weights into the main model.")
            new_dict = match_state_dict(self.model.state_dict(), checkpoint["state_dict"])
            load_state_dict(self.model, new_dict, strict=strict)
    ...
    def save(
        ...
        if not self.is_debug and distutils.is_master():
            ...
            # if we have a teacher model, we want to save the student model instead.
            if (
                hasattr(unwrapped_model, "teacher_model")
                and unwrapped_model.teacher_model is not None
            ):
                if training_state:
                    # save everything to be able to resume training
                    state_dict = self.model.state_dict()
                else:
                    # save a clean student model for inference
                    state_dict = {
                        k: v
                        for k, v in self.model.state_dict().items()
                        if "teacher_model" not in k and "out_mlp_embs" not in k
                    }
            else:
                state_dict = self.model.state_dict()
