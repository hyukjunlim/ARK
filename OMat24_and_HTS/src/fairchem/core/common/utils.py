...
def setup_logging() -> None:
    ...
    target_logging_level = logging.INFO
    # Get the root logger
    root = logging.getLogger()
    # In some cases, a logger may be already set up. If so, we should not add a new one.
    root.setLevel(target_logging_level)
    if not root.hasHandlers():
        warning_log_formatter = logging.Formatter(
            "%(asctime)s %(pathname)s:%(lineno)d: (%(levelname)s): %(message)s",
            datefmt="%Y-%m-%d %H:%M:%S",
        )
        info_log_formatter = logging.Formatter("%(message)s")
        ...
        handler_out.setFormatter(info_log_formatter)
        root.addHandler(handler_out)

        # Send WARNING (and higher) to stderr
        handler_err = logging.StreamHandler(sys.stderr)
        handler_err.setLevel(logging.WARNING)
        handler_err.setFormatter(warning_log_formatter)
...
def new_trainer_context(*, config: dict[str, Any]):
    ...
    trainer_config = {
        ...
        "local_rank": config.get("local_rank", int(os.environ.get("LOCAL_RANK", 0))),
