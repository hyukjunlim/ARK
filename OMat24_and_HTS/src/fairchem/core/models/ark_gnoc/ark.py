...
from .ark_teacher import ARKTeacher


@registry.register_model("ark_gnoc")
class ARK(nn.Module, GraphModelMixin):
    """
    Arguments
    ---------
    num_spherical: int
    ...
    def __init__(
        ...
        **kwargs,  # backwards compatibility with deprecated arguments
        ...
        if teacher_model_args is not None:
            self.teacher_model = ARKTeacher(**teacher_model_args)
            out_mlp_embs = [
                Dense(
                    emb_size_atom,
...
@registry.register_model("ark_gnoc_backbone")
class ARKBackbone(ARK, BackboneInterface):
    @conditional_grad(torch.enable_grad())
...
@registry.register_model("ark_gnoc_energy_and_grad_force_head")
class ARKEnergyAndGradForceHead(nn.Module, HeadInterface):
    def __init__(
        self,
...
@registry.register_model("ark_gnoc_force_head")
class ARKForceHead(nn.Module, HeadInterface):
    def __init__(
        self,
        backbone,
        num_global_out_layers: int,
