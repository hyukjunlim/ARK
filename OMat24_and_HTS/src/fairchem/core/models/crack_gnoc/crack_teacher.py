...
@registry.register_model("crack_gnoc_teacher")
class CrackTeacher(nn.Module, GraphModelMixin):
    """
    Arguments
    ---------
    num_spherical: int
    ...
    def forward(self, data):
        ...
        # Crack
        #########################################################
        outputs["embs_teacher"] = x_E
        outputs["edge_index"] = main_graph["edge_index"]
...
@registry.register_model("crack_gnoc_teacher_backbone")
class CrackTeacherBackbone(CrackTeacher, BackboneInterface):
    @conditional_grad(torch.enable_grad())
...
@registry.register_model("crack_gnoc_teacher_energy_and_grad_force_head")
class CrackTeacherEnergyAndGradForceHead(nn.Module, HeadInterface):
    def __init__(
        self,
...
@registry.register_model("crack_gnoc_teacher_force_head")
class CrackTeacherForceHead(nn.Module, HeadInterface):
    def __init__(
        self,
        backbone,
        num_global_out_layers: int,
