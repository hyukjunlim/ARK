...
class OCPTrainer(BaseTrainer):
    ...
    def train(self, disable_eval_tqdm: bool = False) -> None:
        ...
        for epoch_int in range(start_epoch, self.config["optim"]["max_epochs"]):
            ...
            for i in range(skip_steps, len(self.train_loader)):
                ...
                if self.step % eval_every == 0 or (i + 1) == len(self.train_loader):
    ...
    def _forward(self, batch):
        ...
        if 'embs_student' in out:
            ...
            outputs['embs_teacher'] = out['embs_teacher']
            outputs['embs_student'] = out['embs_student']
            # outputs['per_atom_energy'] = out['per_atom_energy']
            # outputs['per_atom_energy_teacher'] = out['per_atom_energy_teacher']
        #########################################################
    ...
    def _compute_loss(self, out, batch) -> torch.Tensor:
        ...
        if 'embs_student' in out:
            ...
            distill_lmax = self.config["model"]["backbone"]["distill_lmax"]
            teacher_sphere_channels = self.config["model"]["backbone"]["teacher_model_args"]["sphere_channels"]
            embs_student_flat = out['embs_student']
            embs_teacher_flat = out['embs_teacher']

            N = embs_student_flat.shape[0]
            num_coeffs_total = (distill_lmax + 1) ** 2
            ...
            for l in range(distill_lmax + 1):
                start_idx = l * l
                end_idx = (l + 1) * (l + 1)
                num_coeffs_l = end_idx - start_idx
            ...
            if self.config["optim"].get("n2n_coefficient", 1.0) > 0.0:
                loss.append(
                    self.config["optim"].get("n2n_coefficient", 1.0)
                    * total_n2n_loss
                )
        ...
        # # per-atom energy
        # #########################################################
        # if 'per_atom_energy_teacher' in out:
        #     energy_teacher = out['per_atom_energy_teacher']
        #     energy_student = out['per_atom_energy']
        #     per_atom_energy_loss = F.mse_loss(energy_student, energy_teacher)
        #     loss.append(
        #         0.5
        #         * per_atom_energy_loss
        #     )
    ...
    def _compute_metrics(self, out, batch, evaluator, metrics=None):
        ...
        if 'embs_teacher' in out and 'embs_student' in out:
            targets['embs'] = out['embs_teacher']
            out['embs'] = out['embs_student']
