...
from .equiformer_v2_teacher import EquiformerV2Teacher
...
@registry.register_model("crack_backbone")
class EquiformerV2Backbone(nn.Module, GraphModelMixin):
    ...
    def __init__(
        ...
        self.distill_lmax = distill_lmax
        if teacher_model_args is not None:
            self.teacher_model = EquiformerV2Teacher(**teacher_model_args)
            self.out_mlp_embs = FeedForwardNetwork(
                self.sphere_channels,
                self.ffn_hidden_channels,
                teacher_model_args['sphere_channels'],
                self.lmax_list,
                self.mmax_list,
                self.SO3_grid,
    ...
    def forward(self, data: Batch) -> dict[str, torch.Tensor]:
        ...
        outputs = {"node_embedding": x, "graph": graph}
        #########################################################
        # Crack
        #########################################################
        if self.teacher_model is not None:
            N = x.embedding.shape[0]
            student_embs_full = self.out_mlp_embs(x).embedding
            teacher_embs_full = teacher_outputs["embs_teacher"]

            num_coeffs_to_distill = (self.distill_lmax + 1) ** 2

            outputs["embs_student"] = student_embs_full.narrow(
                1, 0, num_coeffs_to_distill
            ).reshape(N, -1)
            outputs["embs_teacher"] = teacher_embs_full.narrow(
                1, 0, num_coeffs_to_distill
            ).reshape(N, -1)
    ...
    def student_params(self):
        ...
        for name, p in self.named_parameters():
            if "teacher_model" not in name and "out_mlp_embs" not in name:
    ...
    def teacher_params(self):
        if self.teacher_model is not None:
            return self.teacher_model.num_params
        return 0

    @property
    def out_mlp_embs_params(self):
        if hasattr(self, "out_mlp_embs") and self.out_mlp_embs is not None:
            return sum(p.numel() for p in self.out_mlp_embs.parameters())
        return 0
