...
from .crack_teacher import CrackTeacher


@registry.register_model("crack_gnoc")
class Crack(nn.Module, GraphModelMixin):
    """
    Arguments
    ---------
    num_spherical: int
    ...
    def __init__(
        ...
        **kwargs,  # backwards compatibility with deprecated arguments
        ...
        # Crack
        #########################################################
        if teacher_model_args is not None:
            self.teacher_model = CrackTeacher(**teacher_model_args)
            out_mlp_embs = [
                Dense(
                    emb_size_atom,
                    teacher_model_args['emb_size_atom'],
                    activation=activation,
                )
            ] + [
                Dense(
            ...
            self.out_mlp_embs = torch.nn.Sequential(*out_mlp_embs)
        else:
    ...
    def forward(self, data):
        ...
        if self.teacher_model is not None:
            ...
            if self.speed_check:
                ...
                teacher_time = end_teacher_time - start_teacher_time
        ...
        #########################################################
        if self.teacher_model is not None:
            outputs["embs_student"] = self.out_mlp_embs(x_E)
            outputs["embs_teacher"] = teacher_outputs["embs_teacher"]
            outputs["per_atom_energy_teacher"] = teacher_outputs["per_atom_energy"]
            outputs["edge_index"] = teacher_outputs["edge_index"]
            
            # outputs["energy"] = teacher_outputs["energy"]
            # outputs["forces"] = teacher_outputs["forces"]

        if self.speed_check:
            torch.cuda.synchronize()
            end_time = time.time()
            student_time = end_time - start_time
            print(f"Time Acceleration: {teacher_time / student_time}", flush=True)
...
@registry.register_model("crack_gnoc_backbone")
class CrackBackbone(Crack, BackboneInterface):
    @conditional_grad(torch.enable_grad())
...
@registry.register_model("crack_gnoc_energy_and_grad_force_head")
class CrackEnergyAndGradForceHead(nn.Module, HeadInterface):
    def __init__(
        self,
...
@registry.register_model("crack_gnoc_force_head")
class CrackForceHead(nn.Module, HeadInterface):
    def __init__(
        self,
        backbone,
        num_global_out_layers: int,
